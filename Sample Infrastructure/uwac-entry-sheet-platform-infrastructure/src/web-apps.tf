data "azuread_application" "pbqa-func-aad" {
    name = "pbqa-func-${terraform.workspace}"
}
...
resource "azurerm_app_service" "web_app" {
    ...
    app_settings = {
        ...
        "Settings:Beas:ApplicationRole"="Entry-Sheet"
        "Settings:Beas:ApplicationNameInternal" = "EntrySheet"       
        "Settings:Beas:ApplicationAreaInternal" = "EntrySheet"
        ...
        "Settings:EntrySheetApi:BaseUrl"="https://${local.api_func_name}-${var.azure_short_region}-${terraform.workspace}.azurewebsites.net/api"
        "Settings:EntrySheetApi:ClientId"=azuread_application.func_aad.application_id
        "Settings:EntrySheetApi:Scope"="https://${local.func_aad_name}-${terraform.workspace}.azurewebsites.net/user_impersonation"
        "Settings:PbqaApi:BaseUrl"=var.pbqa_api_base_url
        "Settings:PbqaApi:ClientId"=data.azuread_application.pbqa-func-aad.application_id
        "Settings:PbqaApi:Scope"="https://pbqa-func-${terraform.workspace}.azurewebsites.net/user_impersonation"
        "Settings:PbqaPlacingBasisTypes"="DEC,E,F,P,R,X"
        "Settings:LineStatuses"= "Incom,INFO,NTU,Promise,QTU,Quote,Decline,Delete,Submission,Written,Expired,Signed"
        "Settings:EnableBISubLimitForGroupClass"="CYBER PRIVACY & TECH"
        "Settings:IgnisApi:BaseUrl"=var.ignis_api_url
        "Settings:IgnisApi:EnvironmentName"=var.ignis_api_environmentName
    }
}
...
resource "azurerm_app_service" "web_app_dr" {
    ...
    app_settings = {
        ...
        "Settings:EntrySheetApi:BaseUrl"="https://${local.api_func_name}-${var.azure_short_region_dr}-${terraform.workspace}.azurewebsites.net/api"
        "Settings:EntrySheetApi:ClientId"=azuread_application.func_aad.application_id
        "Settings:EntrySheetApi:Scope"="https://${local.func_aad_name}-${terraform.workspace}.azurewebsites.net/user_impersonation"
        "Settings:PbqaApi:BaseUrl"=var.pbqa_api_base_url
        "Settings:PbqaApi:ClientId"=data.azuread_application.pbqa-func-aad.application_id
