module "chefserver-vm" {
  ...
  network_security_group_id = "${azurerm_network_security_group.prod_chefsvr_ngs.id}"
  public_ip                 = "false"

  private_ip_address_allocation = "${var.private_ip_address_allocation}"
  private_ip_address            = "${var.private_ip_address_chef_svr}"
...
resource "azurerm_recovery_services_protected_vm" "chefsvr_prod" {
  resource_group_name = "${azurerm_resource_group.app-rg.name}"
  recovery_vault_name = "${azurerm_recovery_services_vault.chef-vault.name}"
  source_vm_id        = "${module.chefserver-vm.vm_id}"
