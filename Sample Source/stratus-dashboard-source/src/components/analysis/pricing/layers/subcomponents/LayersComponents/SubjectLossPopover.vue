            ...
            <b-row>
                <b-col>
                    <b-form-group label="Name:"
                        :label-for="`name-subjectloss-${target}`">
                        <b-form-input :id="`name-subjectloss-${target}`"
                            v-model="subjectLoss.data.name">
                        </b-form-input>
                    </b-form-group>
                </b-col>
            </b-row>
            ...
            <b-tabs pills style="height: 400px;">
                <b-tab title="Description">
                    <b-form-group label="Description:"
                        :label-for="`desc-subjectloss-${target}`">
                        <b-form-textarea :id="`desc-subjectloss-${target}`"
                            v-model="subjectLoss.data.description"
                            placeholder="Add description..."
                            rows="3"
                            no-resize>
                        </b-form-textarea>
                    </b-form-group>
                    <b-button block variant="primary" @click="copyLayer()">Copy Subject Loss</b-button>
                    <template v-if="!deleteCheck">
                        <b-button block variant="danger" @click="startDeleteCheck()">Delete Subject Loss</b-button>
                    </template>
                    <template v-else>
        ...
        watch: {
            subjectLoss: {
                ...
                handler: function(val, oldVal) {
                    this.$store.dispatch('pricing/structure/commitChange', {mutation: 'updateSubjectLoss', data: val})
                }
            }
        },
        methods: {
            ...
            deleteLayer() {
                this.$store.dispatch('pricing/structure/deleteNode', {mutation: 'deleteSubjectLoss', data: this.target})
            },
            copyLayer() {
